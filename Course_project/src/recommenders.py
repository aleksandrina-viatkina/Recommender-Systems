...
class MainRecommender:
    ...
    def __init__(self, data: pd.DataFrame, weighting: bool = True):
        ...
        self.top_purchases = self.data.groupby(['user_id', 'item_id'])['quantity'].count().reset_index()
        self.top_purchases.sort_values('quantity', ascending=False, inplace=True)
        self.top_purchases = self.top_purchases[self.top_purchases['item_id'] != 999999]

        # Топ покупок по всему датасету
        self.overall_top_purchases = self.data.groupby('item_id')['quantity'].count().reset_index()
        ...
        self.bpr_model = self.fit_bpr(self.user_item_matrix)
    ...
    def _get_recommendations(self, user_id, model, N=5):
        """Рекомендации через стардартные библиотеки implicit - ОБЩАЯ ФУНЦИЯ ДЛЯ ПОЛУЧЕНИЯ РЕКОМЕНДАЦИЙ"""

        
#         res = []
        if user_id in self.userid_to_id.keys():
            recs = [self.id_to_itemid[rec[0]] for rec in model.recommend(userid=self.userid_to_id[user_id],
                                                                        user_items=csr_matrix(
                                                                            self.user_item_matrix).tocsr(),
                                                                        N=N, #поставить бОльшее к-во, если будем выбирать уник
                                                                        filter_already_liked_items=False,
                                                                        filter_items=[self.itemid_to_id[999999]],
                                                                        recalculate_user=False)]
            
            # Обработаем повторяющиеся рекомендации (B recs поставить большее к-во, чем N для фильтрации уник товаров)
#             ind = 0
#             for rec in recs:
#                 if rec in res:
#                     continue
...
#                        break
            ...
            # Обработаем ситуацию, если получилось так, что рекомендаций в итоге < 100
            recs = self._extend_with_top_popular(recs, N=N)
            
            
        else:
            #Если холодный новый юзер, у которого вообще нет покупок
            recs = self.overall_top_purchases[:N]
    ...
    def get_als_recommendations(self, user_id, N=5):
        """Рекомендации через стардартные библиотеки implicit"""

#         self._update_dict(user_id=user_id)
        return self._get_recommendations(user_id, model=self.als_model, N=N)
...
#         self._update_dict(user_id=user_id)
    ...
    def get_own_recommendations(self, user_id, N=5):
